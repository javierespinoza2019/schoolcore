...
const categoryDotMap: Record<string, string> = {
  pagos: 'bg-amber-500',
  inscripciones: 'bg-primary-500',
  profesores: 'bg-accent-500',
  alumnos: 'bg-emerald-500',
  sistema: 'bg-secondary-500',
};
    ...
    <div className="max-w-4xl mx-auto space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-foreground-500 mt-0.5">
            {unreadCount > 0 ? `${unreadCount} sin leer de ${notifications.length} notificaciones` : 'Todas las notificaciones leídas'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {unreadCount > 0 && (
            <Button variant="ghost" size="sm" onClick={handleMarkAllRead} icon="ri-check-double-line">
              Marcar todas leídas
            </Button>
          )}
        </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description={showUnread ? 'No tienes notificaciones sin leer.' : 'No se encontraron notificaciones con esos filtros.'}
        />
      ) : (
        <div className="space-y-1">
          {filtered.map((n) => (
            <div
              key={n.id}
              className={`group flex items-start gap-3 rounded-lg border transition-all duration-150 cursor-pointer whitespace-nowrap ${
                n.read
              ...
              role="button"
              tabIndex={0}
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleMarkRead(n.id); }}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className={`text-sm ${n.read ? 'text-foreground-700' : 'text-foreground-900 font-semibold'}`}>
                        {n.title}
                      </p>
                      <p className="text-xs text-foreground-500 mt-0.5 line-clamp-2">{n.description}</p>
                      {n.monto && (
                      ...
                      {n.alumnos && (
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {n.alumnos.map((a: string) => (
                            <span key={a} className="text-2xs bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded-full">{a}</span>
                          ))}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <span className={`text-3xs px-1.5 py-0.5 rounded-full border whitespace-nowrap ${categoryColorMap[n.category] || ''}`}>
                        {n.category}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                      {n.actionLabel && (
                        <button
                          ...
                          className="text-2xs text-primary-600 hover:text-primary-700 font-medium cursor-pointer whitespace-nowrap"
                        >
                          {n.actionLabel}
                        </button>
                      )}
