...
export default function VincularAlumnoModal({ open, onClose, onVincular, parentName, alreadyLinkedIds }: VincularAlumnoModalProps) {
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button variant="primary" size="sm" icon="ri-link" onClick={handleVincular} disabled={!selectedId}>
            Vincular Alumno
          </Button>
        </>
      }
        ...
        {selectedStudent && (
          <div className="p-3 rounded-lg bg-primary-50 border border-primary-200">
            ...
            <div className="flex items-center gap-2 mt-1">
              <img src={selectedStudent.photo} alt={selectedStudent.fullName} className="w-7 h-7 rounded-full object-cover object-top border border-primary-200" />
              <div>
                <p className="text-sm font-semibold text-primary-800">{selectedStudent.fullName}</p>
                <p className="text-2xs text-primary-600">{selectedStudent.level} {selectedStudent.grade}° {selectedStudent.group}</p>
              </div>
            </div>
          </div>
        )}
