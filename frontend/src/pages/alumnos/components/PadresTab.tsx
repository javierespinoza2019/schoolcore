...
export default function PadresTab({ student, onVincularTutor }: PadresTabProps) {
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-foreground-600">
          {student.parents.length} {student.parents.length === 1 ? 'tutor registrado' : 'tutores registrados'}
        </p>
        {onVincularTutor && (
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge variant="primary" size="sm">{parent.relationship}</Badge>
                </div>
                ...
                <div className="flex items-center gap-1 mt-3">
                  <button className="w-7 h-7 flex items-center justify-center rounded-md text-foreground-400 hover:text-foreground-700 hover:bg-secondary-100 transition-colors cursor-pointer" title="Editar tutor">
                    <i className="ri-pencil-line text-sm" />
                  </button>
                  <button className="w-7 h-7 flex items-center justify-center rounded-md text-foreground-400 hover:text-red-600 hover:bg-red-50 transition-colors cursor-pointer" title="Desvincular tutor">
      ...
      {student.parents.length === 0 && (
        <Card>
          <div className="flex flex-col items-center justify-center py-10 text-foreground-400">
            ...
            {onVincularTutor && (
              <Button variant="primary" size="sm" className="mt-3" icon="ri-user-add-line" onClick={onVincularTutor}>
                Vincular Tutor
              </Button>
            )}
          </div>
