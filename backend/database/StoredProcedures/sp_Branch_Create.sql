...
CREATE OR ALTER PROCEDURE dbo.sp_Branch_Create
    @Id UNIQUEIDENTIFIER, @TenantId UNIQUEIDENTIFIER, @Name NVARCHAR(200), @Code NVARCHAR(20), @IsActive BIT = 1,
    ...
    @OperationalStatus NVARCHAR(30)=NULL, @PhotoUrl NVARCHAR(500)=NULL, @CreatedBy UNIQUEIDENTIFIER=NULL
AS BEGIN SET NOCOUNT ON;
    ...
    INSERT INTO dbo.Branch (Id,TenantId,Name,Code,IsActive,Address,City,[State],PostalCode,Phone,Email,TimeZoneId,
        DirectorName,DirectorEmail,DirectorPhone,Capacity,OpenedAt,Area,Levels,OperationalStatus,PhotoUrl,CreatedAt,CreatedBy)
    VALUES (@Id,@TenantId,@Name,@Code,@IsActive,@Address,@City,@State,@PostalCode,@Phone,@Email,@TimeZoneId,
        @DirectorName,@DirectorEmail,@DirectorPhone,@Capacity,@OpenedAt,@Area,@Levels,@OperationalStatus,@PhotoUrl,SYSUTCDATETIME(),@CreatedBy);
