...
CREATE OR ALTER PROCEDURE dbo.sp_Guardian_Create
    ...
    @Status NVARCHAR(30)=N'active', @PhotoUrl NVARCHAR(500)=NULL, @CreatedBy UNIQUEIDENTIFIER=NULL
AS BEGIN SET NOCOUNT ON;
    INSERT INTO dbo.Guardian (Id,TenantId,FirstName,LastName,Email,Phone,Occupation,Address,Status,PhotoUrl,CreatedAt,CreatedBy)
    VALUES (@Id,@TenantId,@FirstName,@LastName,@Email,@Phone,@Occupation,@Address,@Status,@PhotoUrl,SYSUTCDATETIME(),@CreatedBy);
    SELECT g.Id, g.TenantId, g.FirstName, g.LastName, g.Email, g.Phone, g.Occupation, g.Address, g.Status, g.PhotoUrl,
           g.CreatedAt, g.UpdatedAt, CAST(0 AS INT) AS ChildrenCount
